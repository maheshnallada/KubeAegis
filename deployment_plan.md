...
| **rag-ui** (optional) | `npm run --prefix ui-next dev` (dev) / `npm run --prefix ui-next build && npx serve ui-next/dist -p 5173` (prod) | End-user chat interface |
...
| `ui-sg` | 5173 from `alb-sg` | `api-sg` (8080) |
  ...
  "containerDefinitions": [
    {
      ...
      "environment": [
        {"name": "VITE_BACKEND_URL", "value": "https://api.yourdomain.com"},
...
- Keep `requirements-prod.txt` lean. Do not include `ragas`, `sentence-transformers`, or `deepeval` in the production image unless required. The React UI (`ui-next/`) is a separate static build — it does not belong in the Python Docker image.
