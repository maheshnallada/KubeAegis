...
def process_file(file_path: str, filename: str, source_type: str):
    ...
    with logfire.span(f'Processing File - {filename} - {source_type}', file=filename, source=source_type):
        try:
...
def _existing_vector_size(collection_name: str) -> int | None:
    """Return the configured vector size for an existing collection, or None."""
...
def ensure_collection(wipe: bool = False) -> int:
    """
    Create the Qdrant collection at the active embedding dimension.
    Recreate it when --wipe is set or the stored size does not match.
    """
    dim = get_embedding_dim()
    name = settings.QDRANT_COLLECTION
    exists = qdrant_client.collection_exists(name)
    existing_dim = _existing_vector_size(name) if exists else None
    must_recreate = wipe or (existing_dim is not None and existing_dim != dim)

    if must_recreate and exists:
        qdrant_client.delete_collection(name)
        reason = "wipe requested" if wipe else f"dim mismatch ({existing_dim} → {dim})"
        logfire.info(f"Collection '{name}' deleted ({reason}).")
        exists = False
